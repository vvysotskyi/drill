...
public class TestGracefulShutdown extends ClusterTest {
  ...
  private static void setTestDirectories(ClusterFixtureBuilder builder) {
    ...
    builder.configBuilder.put(ExecConstants.SYS_STORE_PROVIDER_LOCAL_PATH, dirTestWatcher.getStoreDir().getAbsolutePath());
    builder.configBuilder.put(ZookeeperPersistentStoreProvider.DRILL_EXEC_SYS_STORE_PROVIDER_ZK_BLOBROOT, dirTestWatcher.getStoreDir().getAbsolutePath());
  }
  ...
  public void testDrillbitTempDir() throws Exception {
    ...
    setTestDirectories(fixtureBuilder);
